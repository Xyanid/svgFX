...
public class HorizontalLineCommandTest {
    ...
    public void dependingOnTheDataTheNextPositionWillBeCorrectlyCalculatedIfTheCommandIsRelative() throws Exception {
        ...
        assertEquals(30.0d, position3.getY(), MINIMUM_DEVIATION);
        ...
        assertEquals(40.0d, position4.getY(), MINIMUM_DEVIATION);
    }
    ...
    public void dependingOnTheDataTheBoundingBoxWillBeCorrectlyCalculatedIfTheCommandIsRelative() throws Exception {
        ...
        assertEquals(30.0d, boundingBox3.get().getY(), MINIMUM_DEVIATION);
        assertEquals(10.0d, boundingBox3.get().getWidth(), MINIMUM_DEVIATION);
        assertEquals(0.0d, boundingBox3.get().getHeight(), MINIMUM_DEVIATION);

        final Optional<Rectangle> boundingBox4 = new HorizontalLineCommand(false, -10.0d).getBoundingBox(new Point2D(-10, 40));

        assertEquals(-20.0d, boundingBox4.get().getX(), MINIMUM_DEVIATION);
        assertEquals(40.0d, boundingBox4.get().getY(), MINIMUM_DEVIATION);
    ...
    public void dependingOnTheDataTheBoundingBoxWillBeCorrectlyCalculatedIfTheCommandIsAbsolute() throws Exception {
        ...
        assertEquals(20.0d, boundingBox2.get().getY(), MINIMUM_DEVIATION);
        assertEquals(20.0d, boundingBox2.get().getWidth(), MINIMUM_DEVIATION);
        assertEquals(0.0d, boundingBox2.get().getHeight(), MINIMUM_DEVIATION);
        ...
        assertEquals(-10.0d, boundingBox3.get().getX(), MINIMUM_DEVIATION);
        assertEquals(30.0d, boundingBox3.get().getY(), MINIMUM_DEVIATION);
        assertEquals(20.0d, boundingBox3.get().getWidth(), MINIMUM_DEVIATION);
        assertEquals(0.0d, boundingBox3.get().getHeight(), MINIMUM_DEVIATION);
